...
import { unlockiOSAudio } from '@/stores/audioStore';

// === iOS Debug: visible overlay that catches rendering errors ===
type ErrorBoundaryProps = { children: ReactNode; onError?: (err: Error) => void };
...
class IOSErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.error) {
      return <div className="fixed inset-0 z-[9999999] bg-black text-white p-4 text-xs font-mono overflow-auto" style={{ pointerEvents: 'auto' }}>
        <div className="text-red-400 text-base mb-2 font-bold">⚠️ iOS Render Error</div>
        <div className="mb-4 whitespace-pre-wrap break-all">{this.state.error.message}</div>
        <div className="text-gray-400">Stack:</div>
        <div className="whitespace-pre-wrap break-all text-gray-300">{this.state.error.stack}</div>
      </div>;
    }
...
import { GRAVITY_ARRAY } from './lib/physicsConfig';
...
export default function App() {
  ...
  const { isLoading, setLoading, currentVideo, setCurrentVideo } = useLoadingStore();
  ...
  const [showMapInstruction, setShowMapInstruction] = useState(true);
  const mapInstructionTimeout = useRef<NodeJS.Timeout | null>(null);
  const [mapOpenedDuringSession, setMapOpenedDuringSession] = useState(false);
  const [gctVisitedDuringSession, setGctVisitedDuringSession] = useState(false);
  const [galleryVisitedDuringSession, setGalleryVisitedDuringSession] = useState(false);
  const [musicVisitedDuringSession, setMusicVisitedDuringSession] = useState(false);
  const [torisVisitedDuringSession, setTorisVisitedDuringSession] = useState(false);
  const [showGctInstruction, setShowGctInstruction] = useState(false);
  const [showGalleryInstruction, setShowGalleryInstruction] = useState(false);
  const [showMusicInstruction, setShowMusicInstruction] = useState(false);
  const [showTorisInstruction, setShowTorisInstruction] = useState(false);
  const gctInstructionTimeout = useRef<NodeJS.Timeout | null>(null);
  const galleryInstructionTimeout = useRef<NodeJS.Timeout | null>(null);
  const musicInstructionTimeout = useRef<NodeJS.Timeout | null>(null);
  const torisInstructionTimeout = useRef<NodeJS.Timeout | null>(null);
  ...
  // Intro video auto-plays on first load; game loads underneath
  useEffect(() => {
    setCurrentVideo(INTRO_VIDEO_URL);
    const start = Date.now();

    const check = setInterval(() => {
      const state = useLoadingStore.getState();
      if (state.isPhysicsReady && state.playerSpawned) {
        clearInterval(check);
  ...
  // One-shot listener: unlock iOS audio on the very first user interaction.
  // Covers: tap/click, keyboard input, and joystick drag (touchmove).
  useEffect(() => {
    const unlock = () => {
      unlockiOSAudio();
      document.removeEventListener('click', unlock, true);
      document.removeEventListener('touchstart', unlock, true);
      document.removeEventListener('touchmove', unlock, true);
      document.removeEventListener('keydown', unlock, true);
    };
    document.addEventListener('click', unlock, true);
    document.addEventListener('touchstart', unlock, true);
    document.addEventListener('touchmove', unlock, { capture: true, passive: true });
    document.addEventListener('keydown', unlock, true);
    return () => {
      document.removeEventListener('click', unlock, true);
      document.removeEventListener('touchstart', unlock, true);
      document.removeEventListener('touchmove', unlock, { capture: true, passive: true } as EventListenerOptions);
      document.removeEventListener('keydown', unlock, true);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const acquirePointerLock = () => {
      ...
      if (document.pointerLockElement || showMusicPlayer || isBTRMapOpen) return;
    ...
    const handlePointerLockChange = () => {
      ...
      // Reacquire pointer lock if lost unexpectedly
      if (!isLocked && !wasEscapePressed && !isClickHandled && 
  ...
  // Show map instruction after player is fully loaded and in-game
  useEffect(() => {
    if (!mobileControls.controls.isMobile && !mapOpenedDuringSession && !isLoading) {
      // Initially hide the instruction
      setShowMapInstruction(false);
      
      // Show instruction with a delay after the player is in-game
      const timer = setTimeout(() => {
        // Add fade-in class to body for CSS transition
        document.body.classList.add('map-instruction-visible');
        setShowMapInstruction(true);
      }, 5000); // 5 second delay after player is in-game
      ...
      return () => {
        ...
        document.body.classList.remove('map-instruction-visible');
      };
    }
  }, [mobileControls.controls.isMobile, mapOpenedDuringSession, isLoading]);

  // Listen for M key press to hide map instruction
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.code === 'KeyM') {
        setShowMapInstruction(false);
        setMapOpenedDuringSession(true);
      }
    };

    window.addEventListener('keydown', handleKeyDown);
    
    // Also listen for map-open event as a backup
    const handleMapOpen = () => {
      setShowMapInstruction(false);
      setMapOpenedDuringSession(true);
    };
    ...
    // Listen for map changes to hide map instruction when leaving central
    const handleMapChanged = (e: CustomEvent) => {
      if (e.detail?.map && e.detail.map !== 'central') {
        setShowMapInstruction(false);
        setMapOpenedDuringSession(true);
      }
    };
    
    window.addEventListener('map-changed', handleMapChanged as EventListener);

    return () => {
      window.removeEventListener('keydown', handleKeyDown);
      window.removeEventListener('map-open', handleMapOpen);
      window.removeEventListener('map-changed', handleMapChanged as EventListener);
    };
  }, []);

  // Listen for the custom event
  useEffect(() => {
    function handleShowGalleryInstruction() {
      // Only show if not visited during this session
      if (!galleryVisitedDuringSession) {
        setGalleryVisitedDuringSession(true);
        setShowGalleryInstruction(true);
        
        // Auto-hide after 10 seconds
        if (galleryInstructionTimeout.current) clearTimeout(galleryInstructionTimeout.current);
        galleryInstructionTimeout.current = setTimeout(() => setShowGalleryInstruction(false), 10000);
      }
    }
    window.addEventListener('show-gallery-instruction', handleShowGalleryInstruction);
    return () => {
      window.removeEventListener('show-gallery-instruction', handleShowGalleryInstruction);
      if (galleryInstructionTimeout.current) clearTimeout(galleryInstructionTimeout.current);
    };
  }, [galleryVisitedDuringSession]);

  // Listen for the GCT instruction event
  useEffect(() => {
    function handleShowGCTInstruction() {
      // Only show if not visited during this session
      if (!gctVisitedDuringSession) {
        setGctVisitedDuringSession(true);
        setShowGctInstruction(true);
        
        // Auto-hide after 10 seconds
        if (gctInstructionTimeout.current) clearTimeout(gctInstructionTimeout.current);
        gctInstructionTimeout.current = setTimeout(() => setShowGctInstruction(false), 10000);
      }
    }
    window.addEventListener('show-gct-instruction', handleShowGCTInstruction);
    return () => {
      window.removeEventListener('show-gct-instruction', handleShowGCTInstruction);
      if (gctInstructionTimeout.current) clearTimeout(gctInstructionTimeout.current);
    };
  }, [gctVisitedDuringSession]);

  // Listen for the music instruction event
  useEffect(() => {
    function handleShowMusicInstruction() {
      // Only show if not visited during this session
      if (!musicVisitedDuringSession) {
        setMusicVisitedDuringSession(true);
        setShowMusicInstruction(true);
        
        // Auto-hide after 10 seconds
        if (musicInstructionTimeout.current) clearTimeout(musicInstructionTimeout.current);
        musicInstructionTimeout.current = setTimeout(() => setShowMusicInstruction(false), 10000);
      }
    }
    window.addEventListener('show-music-instruction', handleShowMusicInstruction);
    return () => {
      window.removeEventListener('show-music-instruction', handleShowMusicInstruction);
      if (musicInstructionTimeout.current) clearTimeout(musicInstructionTimeout.current);
    };
  }, [musicVisitedDuringSession]);

  // Listen for the toris instruction event
  useEffect(() => {
    function handleShowTorisInstruction() {
      // Only show if not visited during this session
      if (!torisVisitedDuringSession) {
        setTorisVisitedDuringSession(true);
        setShowTorisInstruction(true);
        
        // Auto-hide after 10 seconds
        if (torisInstructionTimeout.current) clearTimeout(torisInstructionTimeout.current);
        torisInstructionTimeout.current = setTimeout(() => setShowTorisInstruction(false), 10000);
      }
    }
    window.addEventListener('show-tori-instruction', handleShowTorisInstruction);
    return () => {
      window.removeEventListener('show-tori-instruction', handleShowTorisInstruction);
      if (torisInstructionTimeout.current) clearTimeout(torisInstructionTimeout.current);
    };
  }, [torisVisitedDuringSession]);

  // Handle show message event
  useEffect(() => {
    const handleShowMessage = (e: CustomEvent) => {
      const { message, duration, helpText } = e.detail;
      if (helpText) {
  ...
  // Listen for I key press to hide map-specific instructions
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.code === 'KeyI') {
        // Hide all instruction GIFs when I is pressed
        setShowGctInstruction(false);
        setShowGalleryInstruction(false);
        setShowMusicInstruction(false);
        setShowTorisInstruction(false);
      }
    };

    window.addEventListener('keydown', handleKeyDown);
    
    // Also listen for interactable events as backup
    const handleInteractableOpen = () => {
      setShowGctInstruction(false);
      setShowGalleryInstruction(false);
      setShowMusicInstruction(false);
      setShowTorisInstruction(false);
    ...
    window.addEventListener('interactable-open', handleInteractableOpen);

    return () => {
      window.removeEventListener('keydown', handleKeyDown);
      window.removeEventListener('interactable-open', handleInteractableOpen);
    };
  }, []);

  return (
    <>
      <IOSErrorBoundary onError={(err) => { setError(err); }}>
      <KeyboardControls map={controls}>
        <Canvas
          ...
          onCreated={({ gl }) => {
            ...
            // Safety net: catch WebGL draw errors (especially iOS Safari index-out-of-range)
            const origRender = gl.render.bind(gl);
            gl.render = ((scene: any, camera: any) => {
              try {
                (origRender as Function)(scene, camera);
              } catch (_e: any) {
                console.warn('[iOS] Suppressed render error:', _e?.message);
              }
            }) as typeof gl.render;
          }}
          camera={{
        ...
        >
          <Physics debug={false} gravity={GRAVITY_ARRAY}>
            <Suspense fallback={null}>
              <Player mobileControls={mobileControls.controls} />
              <World />
              {filterEnabled === 1 && (
                <EffectComposer>
            ...
            <PointerLockControls onLock={() => setIsPointerLocked(true)} onUnlock={() => setIsPointerLocked(false)} />
          </Physics>
        </Canvas>
      </KeyboardControls>
      </IOSErrorBoundary>
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ zIndex: 1000 }}>
        ...
        {/* Map instruction */}
        {showMapInstruction && !isMobile && (
          <div className="fixed top-16 right-16 pointer-events-none z-[100005] animate-fade-in">
            <img src="/images/m.gif" alt="Press M to open map" className="h-[5.12rem] w-auto" />
        ...
        {/* GCT instruction */}
        {showGctInstruction && (
          <div className="fixed top-16 right-16 pointer-events-none z-[100005] animate-fade-in-hold">
            <img src="/images/i.gif" alt="Press I for info" className="h-[5.12rem] w-auto" />
          </div>
        )}
        ...
        {/* Toris instruction */}
        {showTorisInstruction && (
          <div className="fixed top-16 right-16 pointer-events-none z-[100005] animate-fade-in-hold">
            <img src="/images/i.gif" alt="Press I for info" className="h-[5.12rem] w-auto" />
          </div>
        )}

        {/* Return to central message */}
        {showReturnMessage && (
        ...
        {/* Transition video overlay - shows map videos during loading before player spawn is confirmed */}
        {currentVideo && isLoading && (
          <div className="fixed inset-0 z-[99998] bg-black" style={{ pointerEvents: 'none' }}>
            <video
