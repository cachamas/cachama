...
const MobileMapSlideshow = () => {
  ...
  const [isInfoBoxOpenedWithI, setIsInfoBoxOpenedWithI] = useState(false);
  ...
  const showToggleButton = isValidMap && !isCentralMap && !isGCTMap && !isGalleryMap && !isOverworldMap && !isMusicMap && isMobileDevice();
  
  // Show BTR map button in central and overworld maps
  const showBTRButton = (isCentralMap || isOverworldMap) && isMobileDevice();
  
  // Show help text when entering relevant maps
  useEffect(() => {
    if ((isGCTMap || isGalleryMap || currentMap === 'toris') && !isMobile) {
  ...
  }, [currentMap, isGCTMap, isGalleryMap, isMobile]);
  ...
  useEffect(() => {
    // Handle gallery navigation from keyboard
    const handleGalleryNavigation = (e: CustomEvent) => {
    ...
    window.addEventListener('gallery-navigation', handleGalleryNavigation as EventListener);
    
    return () => {
      window.removeEventListener('gallery-navigation', handleGalleryNavigation as EventListener);
    };
  }, [showPrevious, showNext]);

  // Hide gallery arrows when the visualizer closes
  useEffect(function handleGalleryClosedEvent() {
  ...
  // Listen for GCT artwork visualizer open event (desktop 'i' key)
  useEffect(() => {
    function handleGCTOpened() {
      if (!isMobileDevice() && isGCTMap) {
        openGCTSlideshow();
      }
    }
    window.addEventListener('gct-opened', handleGCTOpened);
    return () => window.removeEventListener('gct-opened', handleGCTOpened);
  }, [isGCTMap, openGCTSlideshow]);

  // Listen for GCT navigation from keyboard (left/right arrows)
  useEffect(() => {
    const handleGCTNavigation = (e) => {
      if (!isMobileDevice() && isGCTMap && isSlideShowOpen && showInfo) {
        const { direction } = e.detail || {};
        if (direction === 'prev') showPrevious();
        if (direction === 'next') showNext();
      }
    };
    window.addEventListener('gct-navigation', handleGCTNavigation);
    return () => window.removeEventListener('gct-navigation', handleGCTNavigation);
  }, [isGCTMap, isSlideShowOpen, showInfo, showPrevious, showNext]);

  // Listen for 'i' key to open infobox (desktop only)
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      if ((e.key === 'i' || e.key === 'I') && !isMobileDevice() && (currentMap === 'gct' || currentMap === 'gallery' || currentMap === 'toris')) {
        setIsInfoBoxOpenedWithI(true);
      }
    }
  ...
  // Reset when infobox is closed or map changes
  useEffect(() => {
    if (!showInfo || !(currentMap === 'gct' || currentMap === 'gallery' || currentMap === 'toris')) {
      setIsInfoBoxOpenedWithI(false);
    }
  }, [showInfo, currentMap]);

  return (
    <>
      {/* Help text for PC users */}
      {showHelpText && !isMobile && (
      ...
      {/* Music Map Vinyl Display button - only in music map and on mobile */}
      {isMusicMap && isMobileDevice() && (
        <button 
          onClick={() => {
            // Create a synthetic object to trigger vinyl collection display
            const obj = new THREE.Object3D();
            obj.name = 'Box001_Material_#25_0028'; // First vinyl in the collection
            setSelectedObject(obj);
            setShowInfo(true);
      ...
      {/* Gallery navigation controls - shown when info is visible and not on mobile */}
      {showInfo && selectedObject && !isMobileDevice() && (currentMap === 'gct' || currentMap === 'gallery' || currentMap === 'toris') && (
        <div className="fixed left-0 right-0 flex justify-center items-center z-[9999] pointer-events-auto" 
             ...
             key={`gallery-controls-${forceRender}`}
            ...
            <div className="flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
              <button 
                onClick={(e) => {
                  ...
                  if (currentMap === 'gct') {
                    showPrevious();
                  } else if (currentMap === 'gallery' || currentMap === 'toris') {
                    showPrevious(true);
                  }
                }}
                className="w-16 h-16 flex items-center justify-center rounded-full active:opacity-80 hover:opacity-90 touch-manipulation"
