...
// iOS: AudioContext must be created and resumed synchronously inside a user gesture.
// This function should be called from the first tap/click handler on the loading screen
// to unlock audio playback for the entire session.
let _audioUnlocked = false;
export function unlockiOSAudio() {
  if (_audioUnlocked) return;
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !(window as any).MSStream;
  if (!isIOS) { _audioUnlocked = true; return; }
  try {
...
export const useAudioStore = create<AudioState>((set, get) => ({
  ...
  setTrack: (track: Track) => {
    ...
    const newHowl = new Howl({
      ...
      volume: volume,
      autoplay: false,
      html5: false, // Use Web Audio API — avoids exhausting iOS's limited HTML5 Audio pool
      onend: () => {
  ...
  nextTrack: () => {
    ...
    if (oldHowl) {
      oldHowl.fade(volume, 0, CROSSFADE_DURATION);
      nextHowl.fade(0, volume, CROSSFADE_DURATION);
      // Clean up old Howl after crossfade to avoid exhausting the audio pool
      setTimeout(() => { try { oldHowl.unload(); } catch (_) {} }, CROSSFADE_DURATION + 200);
    }
